...
//! `restore_peer_backup` — admit a peer partition into a secure domain.
//!
//! Runs on the destination peer addressed by an outstanding
//! `create_peer_backup` hand-off. Reconstructs the peer (emulator replay or
//! direct `hw` open), runs `sd_restore_peer_backup` over the peer's masked
//! sealing key, the source member's evidence bundle, the policy, the hand-off
//! `pok_peer_backup`, and the domain's current `sd_mk_backup`, then installs the
//! recovered BKS3 as the peer's device-local recovery pair, records the peer as
//! a domain member, and marks the consumed hand-off as joined. No new domain is
//! minted; the peer holds the same BKS3 as the rest of the domain. Gated by the
//! domain's `allow_peer_cloning` policy flag, which the firmware enforces.

use crate::cli::RestorePeerBackupArgs;
...
/// Run `restore_peer_backup`.
pub fn run(ws: &Workspace, args: &RestorePeerBackupArgs) -> Result<()> {
    ...
    // The receiver must be an outstanding (unconsumed) peer hand-off target.
    let handoff_index = domain_manifest
        .handoffs
        .iter()
        .position(|h| h.kind == HandoffKind::Peer && h.destination == receiver && !h.consumed)
        .ok_or_else(|| {
            Error::InvalidArgs(format!(
                "no outstanding peer hand-off for `{receiver}` in secure domain `{domain}`"
            ))
        })?;
    ...
    // The peer hand-off backup addressed to this receiver.
    let peer_backup_path = ws.secure_domain_peer_backup(domain, receiver);
    if !peer_backup_path.exists() {
        return Err(Error::NotFound {
            what: "peer backup",
            path: peer_backup_path.clone(),
        });
    }
    let pok_peer_backup = util::read_file(&peer_backup_path)?;
    ...
    // Resolve and decode the source-peer evidence bundle.
    let source = source_ref.partition.as_str();
    let source_evidence_path = ws.evidence_ref_path(&source_ref);
    if !source_evidence_path.exists() {
        return Err(Error::NotFound {
            what: "source peer evidence",
            path: source_evidence_path.clone(),
        });
    }
    let source_evidence_bytes = util::read_file(&source_evidence_path)?;
    let bundle = EvidenceBundle::decode(&source_evidence_bytes)?;

    // Host-side fast-fail: the source's three chains must certify the source
    // PID key (evidence refs are workspace references, so the source's
    // artifacts live in this same workspace).
    let source_pid_der = util::read_file(&ws.partition_pid_public_key(source))?;
    let source_sec1 = certs::pub_sec1_from_spki(&source_pid_der)
        .map_err(|e| Error::Internal(format!("parse source pid public key: {e}")))?;
    if !bundle.chains_bind_pid(&source_sec1) {
        return Err(Error::Internal(
            "source peer evidence chains do not certify the source pid public key".to_owned(),
        ));
    }
    ...
    // One-shot HSM operation: HPKE-open the peer hand-off with the receiver's
    // key, authenticated by the source-peer evidence, and recover the domain
    // BKS3. The firmware performs the authoritative evidence, SATA-anchor,
    // report-signature, policy-binding and `allow_peer_cloning` checks.
    let restored = bundle
        .with_hsm_evidence(|ev| {
            operating.session.sd_restore_peer_backup(
                &masked,
                ev,
                &policy_bytes,
                &pok_peer_backup,
        ...
        .map_err(|e| Error::hsm("sd_restore_peer_backup", format!("{e:?}")))?;
    ...
    let member_manifest = MemberManifest {
        ...
        role: Role::Member,
        joined_via: "restore_peer_backup".to_owned(),
        source_partition: Some(handoff_source.clone()),
        source_handoff: Some(rel(ws, &peer_backup_path)?),
    ...
    print_summary(receiver, domain, source, &restored);
    Ok(())
}
...
fn print_summary(
    ...
    println!("restore_peer_backup: `{receiver}` joined `{domain}`");
